...
### 💌 ONMU
**2026.05 ~ 2026.06 · 실제 API 계약 기반 약속 큐레이션 서비스**

카카오톡·지도·캘린더·정산 앱에 흩어진 약속 과정을 하나의 흐름으로 묶은 모바일 서비스. Flutter 앱과 Spring Boot API를 실계약으로 연결하고, 정산 금액 계산은 서버를 source of truth로 삼아 정합성을 지켰습니다.

`Flutter` `Spring Boot` `PostgreSQL/PostGIS` `Azure Container Apps` `Sentry`

</td>
<td width="50%" valign="top">

### 🗺️ LALA
**2026.02 ~ 2026.03 · Azure AI RAG 기반 로컬 관광 추천 서비스**

외국인 관광객에게 서울 외 지역 로컬 명소를 추천하는 서비스. 지역 커뮤니티 언급 수를 추천 랭킹에 반영하는 배치 크롤링이 데이터 병목이었고, 이를 Queue 기반 병렬 worker 구조로 개선했습니다.

`iOS (Swift)` `Flask` `Azure Functions` `PostGIS + pgvector` `Power BI`
...
### 🛡️ AutoDetect
**2026.04 · 소스코드 취약점 자동 탐지 VSCode Extension**

MITRE/CWE 공식 문서 기반으로 취약점 위치와 근거를 설명해주는 보안 분석 솔루션. 분산된 클라우드 파이프라인(GCP ↔ Azure Databricks)의 실패를 Logic App 공통 알림 훅으로 추적 가능하게 만들었습니다.

`Azure Databricks` `GCP Cloud Function` `Azure Data Factory` `Delta Lake` `Logic App`

</td>
<td width="50%" valign="top">

### 🧸 BearKTBaker
**2026.07 · 카카오톡 테마 제작 GUI 툴 (개인 프로젝트)**

비개발자도 코드 없이 GUI로 안드로이드/iOS 카카오톡 테마를 제작할 수 있는 Electron 앱. 배포 후 SNS에서 **6K+ 리포스트, 10K+ 저장**을 달성했고, 실제 사용자 피드백을 반영해 버그를 반복적으로 개선했습니다.
...
**Oracle RDBMS 도서관 DB 시스템** `2024.05 ~ 2024.06`
서비스 규칙(대출 가능 권수, 예약 제한, 연체 여부, 중복 대출 방지)을 데이터 정합성으로 다뤄본 팀 프로젝트. 팀장으로 ERD 설계 및 대출·예약·연체 처리 기능 담당. `Oracle` `SQL` `PL/SQL`

**향유 (2m4t)** `2021 하반기`
Spring Boot 기반 사용자 계정 API 구현 초기 백엔드 프로젝트. 회원가입/로그인, JWT 인증, 유저 조회, 비밀번호 변경, 회원 탈퇴 API 구현. `Spring Boot` `JPA` `MySQL` `JWT` `AWS`
...
[![Solved.ac Profile](http://mazassumnida.wtf/api/v2/generate_badge?boj=song_e)](https://solved.ac/song_e/)
</div>
